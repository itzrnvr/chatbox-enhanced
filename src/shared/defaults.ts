...
export function settings(): Settings {
  const providers: { [key: string]: ProviderSettings } = {}
  SystemProviders.forEach((provider) => {
    if (provider.defaultSettings) {
      providers[provider.id] = {
        ...provider.defaultSettings,
        apiKey: '', // Ensure apiKey is always present, even if empty
      }
    }
  })
  ...
  return {
    ...
    providers, // Add the dynamically generated providers object
  }
}
